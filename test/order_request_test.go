...
func TestOrderAwocRequest_Ok(t *testing.T) {
	...
	data, err := acApi.OrderAwocRequest(context.Background(), item)
...
func TestOrderRequest_AgencyAgreementsWithAdditionalBenefits_Ok(t *testing.T) {
	...
	data, err := acApi.OrderRequest(context.Background(), item)
	er := getCustomErrorType()
	st.Expect(t, err, er)
	st.Expect(t, data.BaseResponse.Success.Id, 496446)
}
...
func TestOrderRequest_Error(t *testing.T) {
	...
	item := getOrderRequestObject()
	_, err := acApi.OrderRequest(context.Background(), item)
