...
func TestMealRequest_Ok(t *testing.T) {
	...
	st.Expect(t, len(data.Action[0].Parameters), 1)
	st.Expect(t, data.Action[0].Name, "LIST")
	st.Expect(t, data.Action[0].Parameters[0].MealCode, 0)
	st.Expect(t, data.Action[0].Parameters[0].MealTypeCode, 0)
	st.Expect(t, data.Action[0].Parameters[0].MealName, "швед")
